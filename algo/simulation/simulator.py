...
class Simulator:
    ...
    def draw_axes(self):
        """Draw x/y centimetre labels around the 200 cm movement area."""
        cell_width = c.MAP_WIDTH / c.GRID_SIZE
        cell_height = c.MAP_HEIGHT / c.GRID_SIZE
        ...
        for coordinate in range(0, c.GRID_SIZE + 1, 5):
            ...
            coordinate_cm = round(coordinate * c.ARENA_SIZE_CM / c.GRID_SIZE)
            x_label = self.small_font.render(str(coordinate_cm), True, axis_colour)
            y_label = self.small_font.render(str(coordinate_cm), True, axis_colour)
        ...
        x_axis = self.small_font.render('X coordinate (cm)', True, axis_colour)
        y_axis = self.small_font.render('Y coordinate (cm)', True, axis_colour)
    ...
    def write_telemetry(self):
        """Publish the current simulator state for simulation/monitor.py."""
        actions = []
        for node in self.animation_nodes:
            action = node.prevAction
            actions.append({'gear': 'FORWARD' if action[0] == Gear.FORWARD else 'REVERSE',
                            'steering': action[1].name})
        state = {
            'elapsed_seconds': self.elapsed_seconds,
            'animation_index': self.animation_index,
            'node_count': len(self.animation_nodes),
            'step_cm': self.astar_args['L'],
            'task_state': self.task_state,
            'recognised': len(self.recognised_obstacles),
            'target_images': self.target_images,
            'algorithm': self.ordering_method,
            'actions': actions,
            'leg_ends': sorted(self.leg_end_indexes.keys()),
        }
        try:
            with open(TELEMETRY, 'w') as telemetry_file:
                json.dump(state, telemetry_file)
        except OSError:
            pass
...
# cd /Users/rashiojha/SC2079-MDP-Group-29
# python -m algo.simulation.monitor
# python -m algo.simulation.simulator
# hamilton - image ordering , hybrid - path plan between obstcle
# reeds sheep handle forward reverse steering change spee