...
def obstacle_to_checkpoint(map, obstacle: Obstacle, theta_offset):
    ...
    theta_scan_list = [0]
    # Fixed front-camera distance from the image-facing red edge.
    # Fixed 30 cm front-camera detection distance keeps planning fast and
    # gives the simulator one consistent checkpoint position per image.
...
def find_shortest_time_hamiltonian(map, obstacles, start, theta_offset, astar_args,
    ...
    for obstacle in obstacles:
        ...
        if candidates:
            checkpoints[obstacle] = candidates
        else:
    ...
    # Cache every collision-aware leg between every candidate checkpoint.
    edges = {}
    start_key = 'start'
    for destination in reachable:
        for d_idx, destination_pose in enumerate(checkpoints[destination]):
            edges[(start_key, destination, d_idx)] = plan_leg(start, destination_pose)
    for source in reachable:
        for s_idx, source_pose in enumerate(checkpoints[source]):
            for destination in reachable:
                if source is not destination:
                    for d_idx, destination_pose in enumerate(checkpoints[destination]):
                        edges[(source, s_idx, destination, d_idx)] = plan_leg(source_pose, destination_pose)

    # Held-Karp over (visited images, last image, last checkpoint).  This
    # chooses checkpoint offsets globally, rather than independently.
    count = len(reachable)
    best = {}
    for index, destination in enumerate(reachable):
        for checkpoint_index in range(len(checkpoints[destination])):
            path, travel_time = edges[(start_key, destination, checkpoint_index)]
            if path is not None:
                state_key = (1 << index, index, checkpoint_index)
                best[state_key] = (travel_time, None)

    for mask in range(1, 1 << count):
        for (state_mask, last, last_checkpoint), state in list(best.items()):
            if state_mask != mask:
                continue
            for nxt in range(count):
                if mask & (1 << nxt):
                    continue
                destination = reachable[nxt]
                for next_checkpoint in range(len(checkpoints[destination])):
                    _, travel_time = edges[(reachable[last], last_checkpoint,
                                             destination, next_checkpoint)]
                    if not np.isfinite(travel_time):
                        continue
                    next_key = (mask | (1 << nxt), nxt, next_checkpoint)
                    candidate = state[0] + travel_time
                    if next_key not in best or candidate < best[next_key][0]:
                        best[next_key] = (candidate, (last, last_checkpoint))

    full_mask = (1 << count) - 1
    finishes = [(state[0], state_key) for state_key, state in best.items()
                if state_key[0] == full_mask]
    best_order, best_paths, best_time = [], [], float('inf')
    if finishes:
        travel_time, state_key = min(finishes)
        selected = []
        while state_key is not None:
            mask, last, checkpoint_index = state_key
            selected.append((last, checkpoint_index))
            previous = best[state_key][1]
            if previous is None:
                state_key = None
            else:
                previous_last, previous_checkpoint = previous
                state_key = (mask ^ (1 << last), previous_last, previous_checkpoint)
        selected.reverse()
        best_order = [reachable[index] for index, _ in selected]
        previous_source = start
        previous_key = start_key
        for index, checkpoint_index in selected:
            destination = reachable[index]
            edge = edges[(previous_key, destination, checkpoint_index)] if previous_key == start_key else \
                edges[(previous_key[0], previous_key[1], destination, checkpoint_index)]
            best_paths.append(edge[0])
            previous_key = (destination, checkpoint_index)
