...
class Node():
    def __init__(self, x: float, y: float, theta: float, 
                 prevAction, parent=None, thetaBins=24) -> None:
        self.x = x
        self.y = y
        self.theta = theta
        self.x_g, self.y_g, self.theta_g = self.discretize_position(
            self.x, self.y, self.theta, thetaBins)
        self.parent = parent
...
class HybridAStar():
    def __init__(self, map: OccupancyMap, x_0: float=15, y_0: float=10, theta_0: float=np.pi/2, 
                 x_f: float=15, y_f: float=180, theta_f: float=np.pi/2, theta_offset: float=0, steeringChangeCost=10, gearChangeCost=20,
                    ...
                    cost_mode: str='distance', forward_speed: float=20., reverse_speed: float=15.,
        ...
        assert -np.pi < theta_0 <= np.pi
        assert -np.pi < theta_f <= np.pi
        assert -10 <= x_0 <= 210 and -10 <= y_0 <= 210
        assert -10 <= x_f <= 210 and -10 <= y_f <= 210
        ...
        # ADDED: A physical-time cost model for shortest-time route planning.
    ...
    def transition_cost(self, previous_action, action):
        """Return one motion primitive's cost in distance or calibrated seconds."""
    ...
    def find_path(self):
        ...
        open = PriorityQueue()
        openList = np.full((c.GRID_SIZE, c.GRID_SIZE, self.thetaBins + 1), np.inf)
        closedList = np.full((c.GRID_SIZE, c.GRID_SIZE, self.thetaBins + 1), np.inf)

        open.put((startNode.f, startNode))
        openList[startNode.x_g, startNode.y_g, startNode.theta_g] = 0
        ...
        while not open.empty() and not pathFound:
            ...
            current_index = (currentNode.x_g, currentNode.y_g, currentNode.theta_g)
            openList[current_index] = np.inf
            nodesExpanded += 1

            # CHANGED: Finish only when the best queued goal node is expanded,
            # rather than when a goal child is first generated.
            ...
            for choice in choices:
                ...
                childNode = Node(x_child, y_child, theta_child, prevAction=choice,
                                 parent=currentNode, thetaBins=self.thetaBins)

                childNode.g = currentNode.g + self.transition_cost(currentNode.prevAction, choice)
                if self.cost_mode == 'time':
                    # A straight-line, maximum-speed estimate is admissible.
                    childNode.h = utils.l2(childNode.x, childNode.y, endNode.x, endNode.y) / \
                        max(self.forward_speed, self.reverse_speed)
                ...
                if childNode.x_g < 0 or childNode.x_g >= 40 or \
                    childNode.y_g < 0 or childNode.y_g >= 40:
                    continue

                child_index = (childNode.x_g, childNode.y_g, childNode.theta_g)
                if openList[child_index] <= childNode.g or closedList[child_index] <= childNode.g:
                    continue
                
                open.put((childNode.f, childNode))
                openList[child_index] = childNode.g
            
            closedList[current_index] = currentNode.g
    ...
    def checkPathFound(self, curNode, thetaMargin:float=np.pi/12, targetDistance:float=21, distanceMargin: float=7.5, maxPerpDistance:float=0.5):
        if abs(utils.M(curNode.theta - self.theta_f)) > thetaMargin:
            return False
