...
class task1():
    ...
    def generate_path(self, message):
        ...
        for obstacle in message["data"]["obstacles"]:
            # `dir` is the side of the obstacle carrying the image (E/N/W/S).
            obstacles.append(Obstacle(obstacle["x"] * 2, obstacle["y"] * 2,
                                      obstacle["dir"], int(obstacle["id"])))

        map = OccupancyMap(obstacles)
        robot = message["data"].get("robot", {"x": 1, "y": 1, "dir": "E"})
        start = (robot["x"] * 10, robot["y"] * 10, utils.facing_to_rad(robot["dir"]))
        tsp = Hamiltonian(map, obstacles, *start, 0, 'euclidean', minR)
