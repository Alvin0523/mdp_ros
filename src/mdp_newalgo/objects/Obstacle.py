...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, x_g: int, y_g: int, facing: str, id: int = -1) -> None:
        ...
        sprite_path = Path(__file__).resolve().parent / 'images' / 'obstacle.png'
        self.image = pygame.image.load(str(sprite_path))
        ...
        # The source sprite's red strip is on its south/bottom edge. Rotate the
        # complete square so that strip exactly represents the requested image side.
        if facing == 'N':
