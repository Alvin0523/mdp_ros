...
class Simulator:
    ...
    def draw_image_marker(self, obstacle):
        """Draw an arrow showing the side of an obstacle containing its image."""
        # CHANGED: High-contrast yellow/black arrows stay visible over both the
        # obstacle sprite and the translucent red safety overlay.
        ...
        centre_x, centre_y = utils.coords_to_pixelcoords(x=obstacle_x + 2.5,
                                                          y=obstacle_y + 2.5)
        direction = np.array([np.cos(obstacle.theta), -np.sin(obstacle.theta)])
        start = np.array([centre_x, centre_y]) + direction * 9
        end = np.array([centre_x, centre_y]) + direction * 26
        pygame.draw.line(self.screen, 'black', start, end, 7)
        pygame.draw.line(self.screen, (255, 220, 0), start, end, 3)

        perpendicular = np.array([-direction[1], direction[0]])
        outer_arrowhead = [end,
                     end - direction * 8 + perpendicular * 5,
                     end - direction * 8 - perpendicular * 5]
        inner_arrowhead = [end,
                           end - direction * 7 + perpendicular * 3,
                           end - direction * 7 - perpendicular * 3]
        pygame.draw.polygon(self.screen, 'black', outer_arrowhead)
        pygame.draw.polygon(self.screen, (255, 220, 0), inner_arrowhead)
    ...
    def draw_robot(self, x, y, theta, action=None):
        ...
        half_length = 18
        half_width = 11
        body = [centre + heading * 24,
                centre + heading * 8 + sideways * half_width,
                centre - heading * half_length + sideways * half_width,
                centre - heading * half_length - sideways * half_width,
                centre + heading * 8 - sideways * half_width]
        front_marker = [centre + heading * 24,
                        centre + heading * 5 + sideways * 7,
                        centre + heading * 5 - sideways * 7]

        pygame.draw.polygon(self.screen, (35, 105, 180), body)
        pygame.draw.polygon(self.screen, 'black', body, 2)
        pygame.draw.polygon(self.screen, (255, 220, 0), front_marker)
        pygame.draw.polygon(self.screen, 'black', front_marker, 1)
        ...
        if self.robot_pose is not None:
            ...
            if action is not None:
                ...
                robot_label = self.small_font.render(
                    f'Robot: ({x / 5:.1f}, {y / 5:.1f}), {(np.degrees(theta) % 360):.0f}°',
                    True, (25, 45, 55))
    ...
    def draw_labels(self):
        ...
        image_label = self.small_font.render('Yellow arrow  Image-facing side', True, (25, 45, 55))
        safe_label = self.small_font.render('Red overlay  Safety / no-go area', True, (25, 45, 55))
        robot_label = self.small_font.render('Blue robot + yellow nose  FRONT', True, (25, 45, 55))

        self.screen.blit(title, (895, 72))
        self.screen.blit(subtitle, (895, 98))
        pygame.draw.line(self.screen, (205, 215, 220), (895, 126), (1145, 126), 1)
        pygame.draw.line(self.screen, 'black', (902, 154), (932, 154), 7)
        pygame.draw.line(self.screen, (255, 220, 0), (902, 154), (932, 154), 3)
        ...
        pygame.draw.polygon(self.screen, (255, 220, 0), [(926, 217), (916, 212), (916, 222)])
    ...
    def start_simulation(self):
        ...
        while True:
            ...
            self.draw_axes()
            # ADDED: Show the exact image-facing side for every obstacle.
            for obstacle in self.obstacles:
                self.draw_image_marker(obstacle)
...
if __name__ == "__main__":
    test_maps = get_maps()
    # ADDED: Default assessment scenario contains five image positions.
    map = test_maps[7][:5]
