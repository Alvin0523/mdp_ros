...
def find_shortest_time_hamiltonian(map, obstacles, start, theta_offset, astar_args,
    ...
    from Hybrid A* routes, then every obstacle permutation is evaluated using those
    physical-time edge costs. This is feasible for the required five-image task.
    """
    ...
    for order in itertools.permutations(reachable):
        source = start_key
        paths = []
        total_time = recognition_seconds * len(order)
        for destination in order:
            path, travel_time = edges[(source, destination)]
            if path is None:
        ...
        if total_time < best_time:
            best_order, best_paths, best_time = list(order), paths, total_time
